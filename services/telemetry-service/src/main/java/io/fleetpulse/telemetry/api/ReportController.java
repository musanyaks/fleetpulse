 ...
 * (window functions, FILTER aggregates, first()/last()).
 * All "days" are Africa/Nairobi calendar days; boundaries are computed here
 * and passed as UTC instants.
 */
@RestController
...
public class ReportController {
    ...
    public Map<String, Object> utilization(@RequestParam(required = false) String from,
        ...
        return envelope(r, "utilization", rows);
    }

    /** 2. Driver performance: score from alerts in range + distance, via vehicle→driver map. */
    @GetMapping("/drivers")
    public Map<String, Object> drivers(@RequestParam(required = false) String from,
        ...
        // vehicle → driver (current assignment; the fixed manifest keeps it valid for stored history)
    ...
    /** 3. Fuel: first()/last() over the window + distance → km per fuel-percentage-point. */
    @GetMapping("/fuel")
    public Map<String, Object> fuel(@RequestParam(required = false) String from,
        ...
        return envelope(r, "fuel", rows);
    }

    /** 4. Violations: alert events per vehicle × type × severity. */
    ...
    public Map<String, Object> violations(@RequestParam(required = false) String from,
        ...
        List<Map<String, Object>> rows = jdbc.queryForList("""
            ...
            """.formatted(r.start(), r.end()));
        return envelope(r, "violations", rows);
    }

    /** 5. Geofence activity: entries / exits / dwellings per vehicle. */
    @GetMapping("/geofence")
    public Map<String, Object> geofence(@RequestParam(required = false) String from,
        ...
        List<Map<String, Object>> rows = jdbc.queryForList("""
            ...
            """.formatted(r.start(), r.end()));
        return envelope(r, "geofence", rows);
    }

    /** 6. Daily summary: fleet-wide aggregates per Nairobi calendar day. */
    @GetMapping("/daily")
